...
public class YubiKeyHmacProvider implements ChallengeResponseProvider {
    ...
    public byte[] getResponse(byte[] challenge) throws ChallengeResponseException {
        ...
        try {
            ...
            synchronized(operationLock) {
                ...
                //Clear any interrupt left by a cancel so the calling thread is not affected afterwards
                Thread.interrupted();
            }
        }
    }

    //Interrupts a pending touch wait, so a cancelled prompt does not leave the user waiting for the device timeout
    public void cancel() {
        synchronized(operationLock) {
    ...
    byte[] buildChallengeFrame(byte[] challenge) {
        byte[] frame = new byte[FRAME_SIZE];
        int len = Math.min(challenge.length, SHA1_MAX_BLOCK_SIZE);
        System.arraycopy(challenge, 0, frame, 0, len);

        //A slot configured for variable length challenges finds the end of the challenge by stripping the
        //trailing padding, so the padding byte must differ from the last challenge byte. This matches ykman.
        if(len < SHA1_MAX_BLOCK_SIZE) {
            Arrays.fill(frame, len, SHA1_MAX_BLOCK_SIZE, len > 0 && challenge[len - 1] == 0 ? (byte)1 : (byte)0);
        }
    ...
    private byte[] readResponse(DeviceHandle handle) throws ChallengeResponseException {
        byte[] firstData = waitForSet(handle, RESP_PENDING_FLAG);
    ...
    private byte[] waitForSet(DeviceHandle handle, int mask) throws ChallengeResponseException {
        long sleepMs = 1;
        long sleptMs = 0;
        long maxTimeMs = 1000;
    ...
    private byte[] usbRead(DeviceHandle handle) throws ChallengeResponseException {
        ...
        try {
            ...
            if(result != FEATURE_RPT_SIZE) {
                throw new ChallengeResponseException("Short read from security key: " + result + " of " + FEATURE_RPT_SIZE + " bytes");
            }

            byte[] data = new byte[FEATURE_RPT_SIZE];
            buffer.get(data, 0, FEATURE_RPT_SIZE);
            return data;
        } finally {
            zeroBuffer(buffer);
        }
